...
public:
...

    {
        ...
        bodyMap_ = bodyMap;
        randomError1_ = randomErrorInput1;
        randomError2_ = randomErrorInput2;
    }
private:
    ...
    void updateGuidance( const double time )
    {
      ...
      //  boost::shared_ptr <InvertibleContinuousProbabilityDistribution<double>> statistics::createBoostRandomVariable(const ContinuousBoostStatisticalDistributions boostDistribution, const std::vector <double> &parameters)
        ...
        {
            currentAngleOfAttack_ = (40.0 + randomError1_)  / 180.0 * mathematical_constants::PI;
        }
        else if( Mach < 6.0 )
        {
            currentAngleOfAttack_ = (10.0 + randomError1_) / 180.0 * mathematical_constants::PI;
        }
        else
        {
            currentAngleOfAttack_ = (( 10.0 + (Mach - 6.0) * 5.0 ) + randomError1_) / 180.0 * mathematical_constants::PI;
         ...
         // Ideally change in flight path angle is 0 but if not minimize absolut value of flight path angle rate
            ...
            {
                currentBankAngle_ = acos(flightPathAngleChangeIdeal) + randomError2_/ 180.0 * mathematical_constants::PI;
            }
            else if(flightPathAngleChangePart2 > 0.0)
            {
              currentBankAngle_ =   mathematical_constants::PI + randomError2_/ 180.0 * mathematical_constants::PI;
            }
         ...
         else if(flightPathAngleChangePart2 < 0.0)
         {
            ...
            {
              currentBankAngle_ =  0.0 + randomError2_/ 180.0 * mathematical_constants::PI;
            }
...
private:
    ...
    double randomError1_;
    double randomError2_;
...
//            boost::make_shared< propagators::PropagationTimeTerminationSettings >( simulationEndEpoch );
    ...
    // Define properties of distribution
            ...
            boost::shared_ptr< statistics::RandomVariableGenerator< double > > randomNumberGenerator1 = statistics::createBoostContinuousRandomVariableGenerator(
                        statistics::normal_boost_distribution, parameters, distributionSeed1 );

            double distributionSeed2 = 20.0;
            boost::shared_ptr< statistics::RandomVariableGenerator< double > > randomNumberGenerator2 = statistics::createBoostContinuousRandomVariableGenerator(
                        statistics::normal_boost_distribution, parameters, distributionSeed2 );
  ...
  //  basic_astrodynamics::AccelerationMap accelerationModelMap = createAccelerationModelsMap(bodyMap, AccelerationMap, bodiesToPropagate, centralBodies );
   ...
   //   std::string outputDirectory = tudat_applications::getOutputPath( );
    ...
    input_output::writeDataMapToTextFile( dynamicsSimulator.getDependentVariableHistory( ),
                                          "Results_Dependent_Variables_Q3_" +
